...
// 計算某欄位的數量出現幾次
// column_obj = { name: 'file_id', alias: 'file_id_total' }
exports.count_column = function(table_name, column_obj, cb) {
  conn.query('SELECT ' + column_obj.name + ', COUNT(' + column_obj.name + ') AS ' + column_obj.alias + ' FROM ' + table_name + ' GROUP BY ' + column_obj.name + ' ORDER BY ' + column_obj.alias + ' DESC', function (error, results, fields) {
    if (error) throw error
    cb(results)
  })
  //console.log(query.sql)
